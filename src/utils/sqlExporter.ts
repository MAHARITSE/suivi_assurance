...
export function generateMySQLDump(data: {
  ...
  lines.push(`-- Compatible avec : WAMP Server / XAMPP / MySQL 5.7+ / MySQL 8.0+ / MariaDB / phpMyAdmin`);
  lines.push(`-- ========================================================`);
  lines.push(``);
  ...
  lines.push(`  \`id\` VARCHAR(100) NOT NULL,`);
  lines.push(`  \`nom\` VARCHAR(255) NOT NULL,`);
  lines.push(`  \`code\` VARCHAR(100) NOT NULL,`);
  ...
  lines.push(`  \`data\` LONGTEXT DEFAULT NULL,`);
  ...
  lines.push(`  KEY \`idx_societes_code\` (\`code\`)`);
  lines.push(`) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;`);
  lines.push(``);

  if (data.societes && data.societes.length > 0) {
    lines.push(`-- Contenu de la table \`societes\``);
    lines.push(`INSERT INTO \`societes\` (\`id\`, \`nom\`, \`code\`, \`contact\`, \`telephone\`, \`email\`, \`adresse\`, \`taux_couverture_defaut\`, \`data\`) VALUES`);
    const values = data.societes.map(s => 
      `(${escapeSQL(s.id)}, ${escapeSQL(s.nom)}, ${escapeSQL(s.code)}, ${escapeSQL(s.contact)}, ${escapeSQL(s.telephone)}, ${escapeSQL(s.email)}, ${escapeSQL(s.adresse)}, ${escapeSQL(s.tauxCouvertureDefaut ?? 80)}, ${escapeSQL(s)})`
    );
  ...
  lines.push(`  \`qualite\` VARCHAR(100) DEFAULT 'Adhérent Principal',`);
  lines.push(`  \`famille_code\` VARCHAR(100) DEFAULT NULL,`);
  lines.push(`  \`date_naissance\` VARCHAR(100) DEFAULT NULL,`);
  lines.push(`  \`telephone\` VARCHAR(100) DEFAULT NULL,`);
  lines.push(`  \`email\` VARCHAR(255) DEFAULT NULL,`);
  lines.push(`  \`taux_couverture\` DECIMAL(5,2) DEFAULT NULL,`);
  lines.push(`  \`statut\` VARCHAR(100) DEFAULT 'Actif',`);
  lines.push(`  \`data\` LONGTEXT DEFAULT NULL,`);
  lines.push(`  \`created_at\` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,`);
  lines.push(`  \`updated_at\` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,`);
  lines.push(`  PRIMARY KEY (\`id\`),`);
  lines.push(`  KEY \`idx_personnes_matricule\` (\`matricule\`),`);
  lines.push(`  KEY \`idx_personnes_societe\` (\`societe_id\`),`);
  lines.push(`  CONSTRAINT \`fk_personnes_societe\` FOREIGN KEY (\`societe_id\`) REFERENCES \`societes\` (\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
  lines.push(`) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;`);
  lines.push(``);

  if (data.personnes && data.personnes.length > 0) {
    lines.push(`-- Contenu de la table \`personnes\``);
    lines.push(`INSERT INTO \`personnes\` (\`id\`, \`nom_prenom\`, \`matricule\`, \`societe_id\`, \`sous_societe\`, \`qualite\`, \`famille_code\`, \`date_naissance\`, \`telephone\`, \`email\`, \`taux_couverture\`, \`statut\`, \`data\`) VALUES`);
    const values = data.personnes.map(p => 
      `(${escapeSQL(p.id)}, ${escapeSQL(p.nomPrenom)}, ${escapeSQL(p.matricule)}, ${escapeSQL(p.societeId)}, ${escapeSQL(p.sousSociete)}, ${escapeSQL(p.qualite || 'Adhérent Principal')}, ${escapeSQL(p.familleCode)}, ${escapeSQL(p.dateNaissance)}, ${escapeSQL(p.telephone)}, ${escapeSQL(p.email)}, ${escapeSQL(p.tauxCouverture)}, ${escapeSQL(p.statut || 'Actif')}, ${escapeSQL(p)})`
    );
    lines.push(values.join(',\n') + ';');
    lines.push(``);
  }
  ...
  lines.push(`  \`updated_at\` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,`);
  lines.push(`  PRIMARY KEY (\`id\`),`);
  lines.push(`  KEY \`idx_familles_code\` (\`code\`)`);
  lines.push(`) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;`);
  lines.push(``);

  if (data.familles && data.familles.length > 0) {
    lines.push(`-- Contenu de la table \`familles\``);
    lines.push(`INSERT INTO \`familles\` (\`id\`, \`code\`, \`libelle\`, \`plafond_annuel\`, \`taux_standard\`, \`tarif_conventionne\`, \`ticket_moderateur_defaut\`, \`description\`, \`aliases\`, \`data\`) VALUES`);
    const values = data.familles.map(f => 
      `(${escapeSQL(f.id)}, ${escapeSQL(f.code)}, ${escapeSQL(f.libelle)}, ${escapeSQL(f.plafondAnnuel)}, ${escapeSQL(f.tauxStandard)}, ${escapeSQL(f.tarifConventionne)}, ${escapeSQL(f.ticketModerateurDefaut)}, ${escapeSQL(f.description)}, ${escapeSQL(f.aliases ? JSON.stringify(f.aliases) : '[]')}, ${escapeSQL(f)})`
    );
    lines.push(values.join(',\n') + ';');
    lines.push(``);
  }
  ...
  lines.push(`  \`id\` VARCHAR(100) NOT NULL,`);
  lines.push(`  \`numero_facture\` VARCHAR(255) NOT NULL,`);
  lines.push(`  \`date\` VARCHAR(100) DEFAULT NULL,`);
  lines.push(`  \`societe_id\` VARCHAR(100) NOT NULL,`);
  lines.push(`  \`societe_nom\` VARCHAR(255) DEFAULT NULL,`);
  lines.push(`  \`sous_societe\` VARCHAR(255) DEFAULT NULL,`);
  lines.push(`  \`personne_id\` VARCHAR(100) DEFAULT NULL,`);
  ...
  lines.push(`  \`motif_exclusion\` VARCHAR(255) DEFAULT NULL,`);
  lines.push(`  \`reste_a_payer\` DECIMAL(15,2) DEFAULT 0.00,`);
  lines.push(`  \`statut\` VARCHAR(100) DEFAULT 'En attente',`);
  lines.push(`  \`date_creation\` VARCHAR(100) DEFAULT NULL,`);
  lines.push(`  \`date_paiement\` VARCHAR(100) DEFAULT NULL,`);
  lines.push(`  \`numero_bordereau\` VARCHAR(255) DEFAULT NULL,`);
  lines.push(`  \`commentaires\` TEXT DEFAULT NULL,`);
  lines.push(`  \`data\` LONGTEXT DEFAULT NULL,`);
  lines.push(`  \`created_at\` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,`);
  lines.push(`  \`updated_at\` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,`);
  lines.push(`  PRIMARY KEY (\`id\`),`);
  lines.push(`  KEY \`idx_prestations_facture\` (\`numero_facture\`),`);
  lines.push(`  KEY \`idx_prestations_societe\` (\`societe_id\`),`);
  lines.push(`  KEY \`idx_prestations_personne\` (\`personne_id\`),`);
  lines.push(`  KEY \`idx_prestations_statut\` (\`statut\`),`);
  lines.push(`  CONSTRAINT \`fk_prestations_societe\` FOREIGN KEY (\`societe_id\`) REFERENCES \`societes\` (\`id\`) ON DELETE CASCADE ON UPDATE CASCADE,`);
  lines.push(`  CONSTRAINT \`fk_prestations_personne\` FOREIGN KEY (\`personne_id\`) REFERENCES \`personnes\` (\`id\`) ON DELETE SET NULL ON UPDATE CASCADE`);
  lines.push(`) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;`);
  lines.push(``);

  if (data.prestations && data.prestations.length > 0) {
    lines.push(`-- Contenu de la table \`prestations\``);
    lines.push(`INSERT INTO \`prestations\` (\`id\`, \`numero_facture\`, \`date\`, \`societe_id\`, \`societe_nom\`, \`sous_societe\`, \`personne_id\`, \`nom_agent\`, \`matricule\`, \`total_prestation\`, \`participation\`, \`montant_a_rembourser\`, \`total_paye\`, \`montant_exclu\`, \`motif_exclusion\`, \`reste_a_payer\`, \`statut\`, \`date_creation\`, \`date_paiement\`, \`numero_bordereau\`, \`commentaires\`, \`data\`) VALUES`);
  ...
  lines.push(`  \`id\` VARCHAR(100) NOT NULL,`);
  lines.push(`  \`prestation_id\` VARCHAR(100) NOT NULL,`);
  lines.push(`  \`code\` VARCHAR(100) NOT NULL,`);
  ...
  lines.push(`  CONSTRAINT \`fk_lignes_prestation_prestation\` FOREIGN KEY (\`prestation_id\`) REFERENCES \`prestations\` (\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
  lines.push(`) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;`);
  lines.push(``);
  ...
  if (allLignesPrestation.length > 0) {
    ...
    lines.push(`INSERT INTO \`lignes_prestation\` (\`id\`, \`prestation_id\`, \`code\`, \`libelle\`, \`total_prestation\`, \`ticket_moderateur\`, \`montant_a_rembourser\`, \`total_paye\`, \`montant_exclu\`, \`motif_exclusion\`, \`statut\`, \`data\`) VALUES`);
    const values = allLignesPrestation.map(l => 
      `(${escapeSQL(l.id)}, ${escapeSQL(l.prestationId)}, ${escapeSQL(l.code)}, ${escapeSQL(l.libelle)}, ${escapeSQL(l.totalPrestation ?? l.montant ?? 0)}, ${escapeSQL(l.ticketModerateur ?? 0)}, ${escapeSQL(l.montantARembourser ?? 0)}, ${escapeSQL(l.totalPaye ?? 0)}, ${escapeSQL(l.montantExclu ?? 0)}, ${escapeSQL(l.motifExclusion)}, ${escapeSQL(l.statut || 'En attente')}, ${escapeSQL(l)})`
    );
    lines.push(values.join(',\n') + ';');
    lines.push(``);
  }
  ...
  lines.push(`  \`nom_agent\` VARCHAR(255) DEFAULT NULL,`);
  lines.push(`  \`matricule\` VARCHAR(100) DEFAULT NULL,`);
  lines.push(`  \`prestation_id\` VARCHAR(100) DEFAULT NULL,`);
  lines.push(`  \`prestation_numero\` VARCHAR(255) DEFAULT NULL,`);
  lines.push(`  \`mode_paiement\` VARCHAR(100) DEFAULT 'Virement bancaire',`);
  lines.push(`  \`reference_paiement\` VARCHAR(255) DEFAULT NULL,`);
  ...
  lines.push(`  \`statut\` VARCHAR(100) DEFAULT 'Validé',`);
  ...
  lines.push(`  \`id\` VARCHAR(100) NOT NULL,`);
  lines.push(`  \`paiement_id\` VARCHAR(100) NOT NULL,`);
  lines.push(`  \`ligne_prestation_id\` VARCHAR(100) DEFAULT NULL,`);
  lines.push(`  \`prestation_id\` VARCHAR(100) DEFAULT NULL,`);
  ...
  lines.push(`  \`code_acte\` VARCHAR(100) DEFAULT NULL,`);
  ...
  lines.push(`  KEY \`idx_lp_prestation_ref\` (\`prestation_id\`),`);
  lines.push(`  KEY \`idx_lp_ligne_prestation_ref\` (\`ligne_prestation_id\`),`);
  lines.push(`  CONSTRAINT \`fk_lignes_paiement_paiement\` FOREIGN KEY (\`paiement_id\`) REFERENCES \`paiements\` (\`id\`) ON DELETE CASCADE ON UPDATE CASCADE,`);
  lines.push(`  CONSTRAINT \`fk_lignes_paiement_prestation\` FOREIGN KEY (\`prestation_id\`) REFERENCES \`prestations\` (\`id\`) ON DELETE SET NULL ON UPDATE CASCADE,`);
  lines.push(`  CONSTRAINT \`fk_lignes_paiement_ligne_prestation\` FOREIGN KEY (\`ligne_prestation_id\`) REFERENCES \`lignes_prestation\` (\`id\`) ON DELETE SET NULL ON UPDATE CASCADE`);
  lines.push(`) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;`);
  lines.push(``);
  ...
  if (allLignesPaiement.length > 0) {
    ...
    lines.push(`INSERT INTO \`lignes_paiement\` (\`id\`, \`paiement_id\`, \`ligne_prestation_id\`, \`prestation_id\`, \`immatriculation\`, \`nom_base_assurance\`, \`nom_agent\`, \`prestation_numero\`, \`date_soins\`, \`total_paye\`, \`ticket_moderateur\`, \`montant_exclu\`, \`montant_reclame\`, \`code_acte\`, \`libelle_acte\`, \`actes_payes\`, \`commentaire\`, \`data\`) VALUES`);
    const values = allLignesPaiement.map(l => 
      `(${escapeSQL(l.id)}, ${escapeSQL(l.paiementId)}, ${escapeSQL(l.lignePrestationId)}, ${escapeSQL(l.prestationId)}, ${escapeSQL(l.immatriculation)}, ${escapeSQL(l.nomBaseAssurance)}, ${escapeSQL(l.nomAgent)}, ${escapeSQL(l.prestationNumero)}, ${escapeSQL(l.dateSoins)}, ${escapeSQL(l.totalPaye ?? l.montantPaye ?? 0)}, ${escapeSQL(l.ticketModerateur ?? 0)}, ${escapeSQL(l.montantExclu ?? 0)}, ${escapeSQL(l.montantReclame ?? 0)}, ${escapeSQL(l.codeActe)}, ${escapeSQL(l.libelleActe)}, ${escapeSQL(l.actesPayes || [])}, ${escapeSQL(l.commentaire)}, ${escapeSQL(l)})`
    );
    lines.push(values.join(',\n') + ';');
    lines.push(``);
  }
  ...
  lines.push(`-- ========================================================`);
  lines.push(`-- FIN DU DUMP SQL MYSQL`);
  lines.push(`-- ========================================================`);
