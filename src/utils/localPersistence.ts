...
import { initialSocietes, initialFamilles, initialPersonnes, initialPrestations, initialPaiements } from '../data/initialData';
...
export function loadLocalDataset(): {
  ...
  try {
    ...
    const personnes: Personne[] = pRaw ? JSON.parse(pRaw) : initialPersonnes;
    const familles: Famille[] = fRaw ? JSON.parse(fRaw) : initialFamilles;
    const rawPrestations: Prestation[] = prRaw ? JSON.parse(prRaw) : initialPrestations;
    // Ensure unlinked/autogenerated dummy payments do not clutter the prestations list
    const prestations: Prestation[] = rawPrestations.filter(p => 
      !p.id.startsWith('prest-autogen') && 
      !p.commentaires?.includes('Prestation générée lors du règlement')
    );
    const paiements: Paiement[] = paRaw ? JSON.parse(paRaw) : initialPaiements;
